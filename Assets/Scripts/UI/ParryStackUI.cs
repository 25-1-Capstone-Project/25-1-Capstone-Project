...
public class ParryStackUI : MonoBehaviour
{
    ...
    public void AddParryStackIcon()
    {
      //  Debug.Log("ParryStackUI AddParryStackIcon() currentParryStack: " + PlayerScript.Instance.GetPlayerRuntimeStats().currentParryStack);
        parryStacksIcons[ GameManager.Instance.playerScript.GetPlayerRuntimeStats().currentParryStack - 1].SetActive(true);
    }
    public void RemoveParryStackIcon(int cost)
    {

        int index =  GameManager.Instance.playerScript.GetPlayerRuntimeStats().currentParryStack;
    ...
    public void SetMaxParryStack()
    {
        ...
        for (int i = 0; i < GameManager.Instance.playerScript.GetPlayerRuntimeStats().maxParryStack; i++)
        {
